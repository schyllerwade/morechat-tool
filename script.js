...
// Token elements
const originalTokensResult = document.getElementById('originalTokensResult');
const compressedTokensResult = document.getElementById('compressedTokensResult');
...
// Approximates tokens using word count (1 word ≈ 1.3 tokens)
function estimateTokens(text) {
    const words = text.trim().split(/\s+/).filter(w => w.length > 0).length;
    return Math.max(0, Math.round(words * 1.3));
}

// Update word, character, and token count
...
async function handleCompress() {
    ...
    // Display input tokens during loading
    document.getElementById('inputTokensDisplay').textContent = inputTokens.toLocaleString();
    ...
    try {
        ...
        const tokenReduction = inputTokens - outputTokens;
        const reductionPercent = Math.round((1 - outputTokens / inputTokens) * 100);
        const compressionRatio = Math.round((outputTokens / inputTokens) * 100);
        ...
        const savedCost = inputCost - ((outputTokens / 1000000) * 0.150);
        ...
        reductionPercent.textContent = reductionPercent;
        
        // Update token display
        originalTokensResult.textContent = inputTokens.toLocaleString();
        compressedTokensResult.textContent = outputTokens.toLocaleString();
        tokensSaved.textContent = tokenReduction.toLocaleString();
        
        // FIX: Update reduction percentage correctly
        if (reductionPercentDisplay) {
            reductionPercentDisplay.textContent = reductionPercent;
        }
        
        // Update cost saved
        costSaved.textContent = '$' + Math.max(0, savedCost).toFixed(4);
        
        // Animate meter bar
        meterBarFill.style.width = compressionRatio + '%';
        compressionPercentage.textContent = reductionPercent + '% ✨';
        ...
        document.getElementById('compressionLevel').textContent = levelDisplay[level];
...
async function compressConversation(text, level, target, apiKey) {
    ...
    const systemPrompt = `You compress AI conversations. Keep key decisions, facts, and context. Remove chitchat and redundancy. Extract action items.

${compressionInstructions[level]}${targetInstructions[target]}

Format: PROJECT CONTEXT | KEY DECISIONS | FACTS | STATUS | ACTION ITEMS`;
